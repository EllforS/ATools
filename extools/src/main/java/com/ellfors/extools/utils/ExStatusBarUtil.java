...
public class ExStatusBarUtil
{
    ...
    private static final int MIN_API = 17;
    ...
    private static void darkMode(Window window, int color, @FloatRange(from = 0.0, to = 1.0) float alpha)
    {
        ...
        {
            window.addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            setTranslucentView((ViewGroup) window.getDecorView(), Color.BLACK, 1f);
        }
        else
        {
            immersive(window, Color.BLACK, 1f);
        }
    }
     ...
     * 不支持沉浸式
     */
    public static void darkModeForBase(Activity activity, int color, @FloatRange(from = 0.0, to = 1.0) float alpha)
    {
        Window window = activity.getWindow();
        if (isFlyme4Later())
        {
            darkModeForFlyme4(window, true);
            setBaseStatusBar(activity, color, alpha);
        }
        else if (isMIUI6Later())
        {
            darkModeForMIUI6(window, true);
...
//            window.addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
//            setTranslucentView((ViewGroup) window.getDecorView(), Color.BLACK, 1f);
            setBaseStatusBar(activity, Color.BLACK, 1f);
        }
        else
        {
            setBaseStatusBar(activity, Color.BLACK, 1f);
     ...
     * @param alpha    状态栏透明度
     */
    public static void setBaseStatusBar(Activity activity, @ColorInt int color, @FloatRange(from = 0.0, to = 1.0) float alpha)
    {
     ...
     * @return 状态栏矩形条
     */
    private static View createStatusBarView(Activity activity, @ColorInt int color, float alpha)
    {
        // 绘制一个和状态栏一样高的矩形

...
public class ExViewUtil
{
    ...
    //过滤Emoji表情
    public static Pattern emoji = Pattern.compile("[\ud83c\udc00-\ud83c\udfff]|[\ud83d\udc00-\ud83d\udfff]|[\u2600-\u27ff]", Pattern.UNICODE_CASE | Pattern.CASE_INSENSITIVE);
    //过滤特殊字符
    public static Pattern speChat = Pattern.compile("[`~!@#$%^&*()+=|{}':;',\\[\\].<>/?~！@#￥%……&*（）——+|{}【】‘；：”“’。，、？]");
    //只能输入中英文•
     ...
     * 设置editText 只能输入中文
     */
    public static void setInputOnlyChinese(EditText editText, @IntRange(from = 0, to = Integer.MAX_VALUE) int maxLength)
    {
        InputFilter filter = new InputFilter()
        {
            ...
            {
                Matcher chineseMatcher = chinese.matcher(source.toString());
    ...
    public static int getStatusBarHeight(Context context, int actionBarHeight)
    {
        ...
        result += ExDensityUtil.dp2px(context, actionBarHeight);
        return result;
    }
    ...
    public static void setLeftIcon(Activity context, TextView textView, int icon, int drawablePadding)
    {
        ...
        else
        {
            ...
            textView.setCompoundDrawablePadding(dip2px(context, drawablePadding));
        }
    }
